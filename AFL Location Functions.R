...
get_match_chains <- function(season = year(Sys.Date()), round = NA) {
  ...
  chains <- chains %>% dplyr::select(season,roundNumber,homeTeam.teamName:date,venue.name,venueWidth:homeTeamDirectionQtr1,displayOrder,chain_number,chainTeamName,initialState,finalState,period,periodSeconds,playerName.givenName:team.teamName,description,disposal:y) %>%
...
teamIds <- tibble(chainTeamName = c("Adelaide Crows", "Brisbane Lions", "Carlton", "Collingwood", "Essendon",
